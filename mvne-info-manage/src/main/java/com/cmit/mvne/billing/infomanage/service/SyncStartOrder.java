...
public class SyncStartOrder {
    ...
    public void sync(IOrdOrder iOrdOrder,IUser iUser) throws MvneException {
        System.out.println(Thread.currentThread().getName() + "-start- " + iOrdOrder.getOrderId());

        Long orderId = iOrdOrder.getOrderId();
        log.info("Sync start user order : {}", orderId);
     ...
     * @throws Exception
     */
    public void syncDB(IUser iUser) throws Exception {
        //修改用户资料的失效时间修改为当前时间
        int updateStatus = cmUserDetailService.updateExpire(new CmUserDetail(iUser).getUserId(), iUser.getMsisdn(),iUser.getValidDate(), "11");
        if (updateStatus >= 1) {
     ...
     * @throws Exception
     */
    public void syncRedis(IUser iUser) throws Exception {
        //获取用户资料key
    ...
    public Boolean updateExpireDate(List<CmUserDetail> cmUserDetailList,Long userId,String msisdn,Date validDate,String userTableKey) throws ParseException {
        ...
        for (CmUserDetail cmUserDetail : cmUserDetailList) {
            if (cmUserDetail.getUserId().equals(userId) && cmUserDetail.getMsisdn().equals(msisdn) && simpleDateFormat.format(cmUserDetail.getExpireDate()).equals(simpleDateFormat.format(expireDate)) && cmUserDetail.getUserStatus().equals("11")) {
                //更新失效时间
